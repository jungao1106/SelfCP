...
class ModelModule(nn.Module):
    def __init__(self, modelNameOrPath, options=[], mlpPath=None, embeddingPath=None, normPath=None) -> None:
        ...
        self.__model = LlamaForCausalLM.from_pretrained(modelNameOrPath, torch_dtype=torch.bfloat16, trust_remote_code=True)
    ...
    def cprsedcot(self, inputs, c_inputs, op):
        with torch.no_grad():
            ...
            for inputIds in recInputIds:
                ...
                cprsedHiddenStates = self.__norm(self.__mlp(cprsedHiddenStates)).to(torch.bfloat16)
    ...
    def baseline(self, inputs):
        with torch.no_grad():
            ...
            if 'tail' in inputs.keys():
                
                start = 0
                end = start + 512 - len(outputs[0]) - len(inputs['head']) - len(inputs['tail']) - len(inputs['prompt'])
                inputs['head'] = [item.item() for item in inputs['head']]
                inputs['tail'] = [item.item() for item in inputs['tail']]
                inputs['prompt'] = [item.item() for item in inputs['prompt']]
                inputs['tailContext'] = [item.item() for item in inputs['tailContext']]
                accOuts = outputs.squeeze().tolist()
                while start < len(inputs['tailContext']):
                    nextIds = inputs['head'] + inputs['prompt'] + accOuts + inputs['tailContext'][start: end] + inputs['tail']
                    ...
                    start = end
                    end = start + 512 - len(accOuts) - len(inputs['head']) - len(inputs['tail']) - len(inputs['prompt'])
    ...
    def inference(self, op, inputs, c_inputs):
        with torch.no_grad():
            ...
            c_inputs['cprsed_embeds'] = self.__norm(self.__mlp(cprsedHiddenStates).to(torch.bfloat16))
            decodingTS = time()
    ...
    def timecompute(self):
        for i in range(1000):
            input_ids = torch.randint(5, 32000, (1, 1024))
            c_inputs = torch.randint(5, 32000, (1, 1024))
            input_ids[:, 0] = 1
            c_inputs[:, 0] = 1
            # input_ids[:, -79: ] = 4
            # c_inputs[:, :79] = 4
            batch = {
                'inputs': {'input_ids':input_ids.to(self.__model.device)},
                'c_inputs': {'input_ids': c_inputs.to(self.__model.device)}
            }
            with torch.no_grad():
                # oriHiddenStates = self.__model.model(**batch['inputs'])[0]
                # cprsed_postion = (batch['inputs']['input_ids'] == 4)
                # cprsedHiddenStates = oriHiddenStates[cprsed_postion, ...].view(-1, oriHiddenStates.shape[-1])
                # cprsedHiddenStates = self.__mlp(cprsedHiddenStates).to(torch.bfloat16)
                
                # batch['c_inputs']['cprsed_embeds'] = self.__norm(cprsedHiddenStates)
                _ = self.__model.generate(**batch['c_inputs'],
                                        min_new_tokens = 128,
                                        max_new_tokens = 128, 
                                        num_beams=1,
                                        do_sample = False)
    ...
    def retrieve(self, anchorInputs, rerankInputs, k):
        ...
        anchorHiddenStates = self.__norm(self.__mlp(anchorHiddenStates)).to(torch.bfloat16)
        ...
        for inputs in rerankInputs:
            ...
            rankHiddenStates = self.__norm(self.__mlp(rankHiddenStates)).to(torch.bfloat16)
